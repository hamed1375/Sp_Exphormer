...
def proteins_node_one_hot(data):
    """For ogbn-proteins, move the attribute node_species to attribute x."""
    uniq_ids = data.node_species.unique().tolist()
    index_mapping = {node_id: i for i, node_id in enumerate(uniq_ids)}
    data.x = data.node_species.apply_(index_mapping.get)
    data.x = torch.nn.functional.one_hot(data.x, num_classes=8).squeeze().contiguous()
    return data
...
def add_layer_edge_indices(data, dir, layers):
    device = data.edge_index.device
    for i in range(layers):
        # layer_edge_index = torch.load(dir+f'/edge_index_layer_{i}.pt', map_location=device)
        for j in range(50):
            layer_edge_index = np.load(dir+f'/edge_index_layer_{i}_sample_{j}.npy')
            layer_edge_attr = np.load(dir+f'/edge_attr_layer_{i}_sample_{j}.npy')
            setattr(data, f'edge_index_layer_{i}_sample_{j}', torch.from_numpy(layer_edge_index).to(device))
            setattr(data, f'edge_attr_layer_{i}_sample_{j}', torch.from_numpy(layer_edge_attr).to(device))

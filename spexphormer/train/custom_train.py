...
def custom_train_initial(loggers, loaders, model, optimizer, scheduler):
    ...
    if cfg.train.save_attention_scores:
        cfg.train.saving_epoch=True
        load_ckpt(model)
        model.eval()
        cfg.train.cur_epoch = int(best_epoch)
        os.makedirs(f'Attention_scores/{cfg.dataset.name}', exist_ok=True)
        with open(f'Attention_scores/{cfg.dataset.name}/edges.npy', 'wb') as f:
            np.save(f, loaders[0].dataset.data.edge_index.detach().numpy()) 
        with open(f'Attention_scores/{cfg.dataset.name}/edge_attr.npy', 'wb') as f:
            np.save(f, loaders[0].dataset.data.edge_attr.detach().numpy()) 
        with torch.no_grad():
            for batch in loaders[1]:
                batch.split = 'val'
                batch.to(torch.device(cfg.accelerator))
                _ = model(batch)
        cfg.train.saving_epoch=False
